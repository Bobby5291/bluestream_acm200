...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict | None = None):
        if user_input is not None:
            ...
            self._config = {
                CONF_HOST: host,
                CONF_PORT: int(port),
                CONF_NUM_INPUTS: int(user_input.get(CONF_NUM_INPUTS, DEFAULT_NUM_INPUTS)),
                CONF_NUM_OUTPUTS: int(user_input.get(CONF_NUM_OUTPUTS, DEFAULT_NUM_OUTPUTS)),
    ...
    async def async_step_names(self, user_input: dict | None = None):
        ...
        fields: dict = {}
        for i in range(1, num_inputs + 1):
            fields[vol.Optional(f"in_{i}", default="")] = str
        for o in range(1, num_outputs + 1):
            fields[vol.Optional(f"out_{o}", default="")] = str
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    """Options flow to rename inputs/outputs."""
